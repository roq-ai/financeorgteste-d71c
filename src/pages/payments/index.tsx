...
import { getPayments, deletePaymentsById } from 'apiSdk/payments';
import { PaymentsInterface } from 'interfaces/payments';

type ColumnType = ColumnDef<PaymentsInterface, unknown>;
...
export function PaymentsListPage(props: PaymentsListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PaymentsInterface>>(
  ...
  const handleView = (row: PaymentsInterface) => {
    if (hasAccess('payments', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          ...
          {hasAccess('payments', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
          ...
          {hasAccess('payments', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="payments" />
        </Flex>

        {hasAccess('payments', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
